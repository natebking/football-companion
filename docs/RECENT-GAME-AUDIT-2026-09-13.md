...
The release preserves GitHub’s September 8 scheduled college tendency refresh (`b033721`): 2023–2026, including a partial current season and 373,073 eligible plays. NFL remains 2023–2025. Replay uses current historical tables, which may postdate a replayed game; its percentages are not frozen historical forecasts. The prefix-selection audit uses `history-cfb.json`/`history-nfl.json`, not the refreshed tendency table, so this integration does not alter its measured read selections.

Production verification: code commit `adf881f`, deployment `dpl_GxGx1EPb8oPG6dJNHuKVqhkCNBg9`, ready at https://fluentin.football/ on September 13. Preview and production match local 44-asset manifest `2894330f25794ae07c24ef7ab323e78cd871621f04d7bd51dff1f76686731a71`. Public HTML, app, replay adapter/styles, play parser, refreshed college table and data page hashes match; `/stopwatch` remains byte-identical to the frozen root file. Real Chrome production checks verified the new picker styling, LSU search, loading Louisiana Tech–LSU at zero reports, a Quarter 2 jump and returning to the start with unknown scores. All 281 tests passed after integrating the scheduled data refresh.
